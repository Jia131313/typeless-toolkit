...
- `package.json` 当前版本为 1.8.2；本轮适配 Typeless 2.8.0 的 ASAR 完整性、客户端接口校验与引导状态，
  待完成本机候选验收后再经 PR 与 CI 发布。macOS Tauri 四包迁移（1.8.0）已完成发布与本机安装验收。
...
- 本地试用版不得因构建或安装成功直接发布；先完成用户验收，再按明确授权推进 PR、合并与 Release。

## 更新入口约定

- 「设置 → 关于与更新」分别显示工具集更新与 Typeless 本体更新。Windows 隐藏本体更新入口；
  macOS 本体入口仅检查和安装 Typeless updater 已下载的本地缓存包，不在线发现官方最新版。
- 工具集自更新从 GitHub Release 检查与下载，必须与本体更新使用独立入口、状态和命名。
- 首页或设置中的升级提示只反映实际检测结果；点击打开对应更新窗口，不直接开始下载或安装。
