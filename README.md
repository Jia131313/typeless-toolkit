...
| 词库同步 | 管理器 / CLI | 导出各账号词库到主 CSV,全量对账并分批回灌缺词,回拉后二次校验 |
...
- **去弹窗补丁(实验性)**:修改前会把完整 `Typeless.app` 备份到工具集数据目录下的
  ...
  ad-hoc 签名的默认指定要求会随内容变化，因此更新工具集或重新打补丁后，TCC 中可能留下“开关仍
  开启但新进程无法识别”的旧权限身份。工具集内的“macOS 权限说明”会分别解释：工具集需要 App
  管理权限，Typeless 本体需要辅助功能和麦克风，三项不要求严格先后顺序；并提供一键清除旧权限
  记录的入口，代替在系统设置里手工选中并点击减号。清除只影响授权记录，不会删除账号或应用数据。
