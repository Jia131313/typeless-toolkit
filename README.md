...
**Q: 账号同步需要工具集每天运行吗?**
A: 不需要。账号库会在工具集启动、添加/更新/删除账号、在本机启用云端账号或点击「立即同步」时处理。短期 access token 不上传；需要调用 API 或在新电脑启用账号时，工具才用 refresh token 即时换取新的 access token。

**Q: WebDAV 会上传什么，安全吗?**
A: 只上传账号 ID、昵称、邮箱、客户端用户 ID、refresh token 和删除记录。远端文件使用同步密码经 scrypt 派生密钥后以 AES-256-GCM 加密，不上传 access token、设备 ID、词库或 `profiles/` 登录快照。WebDAV 应用密码和同步密码仅保存在本机数据目录；同步密码遗失后无法恢复远端账号库。坚果云可直接使用预设 URL，用户名填邮箱、密码填坚果云生成的应用密码。Google Drive 和 OneDrive 需要各自的 OAuth 接入，不属于通用 WebDAV，本版暂不提供。
