...
/**
 * API host 可能被内联;也允许用官网地址和环境名识别字符串池。
 */
function locateStringPool(src) {
  for (const anchor of [`'${API_HOST}'`, "'https://www.typeless.com'"]) {
    let idx = src.indexOf(anchor);
    while (idx !== -1) {
      const start = src.lastIndexOf('=[', idx);
      if (start > 0) {
        const pool = evalArrayAt(src, start);
        if (Array.isArray(pool) && (pool.includes(API_HOST) ||
          (pool.includes('https://www.typeless.com') && pool.includes('prod')))) return pool;
      }
...
function extractSignatureKeys(asarBuf) {
  ...
  // 同一配置的另一种输出: aes=F(i),hmac=F(j),env='prod',dev=env!==F(k)。
  const inlinedEnv = !seq;
  if (!seq) seq = src.match(
    /(\w+)=(\w+)\((0x[0-9a-f]+)\),(\w+)=\2\((0x[0-9a-f]+)\),(\w+)='prod',(\w+)=\6!==\2\((0x[0-9a-f]+)\)/
  );
  if (!seq) throw new Error('未能在主进程 bundle 中定位签名参数声明');
  const vnIdx = parseInt(seq[3], 16);
  const euIdx = parseInt(seq[5], 16);
  const diIdx = parseInt(seq[inlinedEnv ? 8 : 7], 16);

  // API host 内联时,使用紧随其后的官网地址索引校准。
  const hostDecl = src.match(/(\w+)=(\w+)\((0x[0-9a-f]+)\),\w+='https:\/\/www\.typeless\.com'/);
  const inlineHost = src.match(/(\w+)='https:\/\/api\.typeless\.com',\w+=(\w+)\((0x[0-9a-f]+)\),(\w+)=\2\((0x[0-9a-f]+)\)/);
  if (!hostDecl && !inlineHost) throw new Error('未能定位 API host 声明,无法校准字符串池');
  const hostIdx = parseInt((hostDecl || inlineHost)[3], 16);
  const anchor = hostDecl ? API_HOST : 'https://www.typeless.com';

  // 版本号紧随官网地址声明之后,不依赖某个具体版本。
  const verDecl = src.match(/\w+='https:\/\/www\.typeless\.com',(\w+)=\w+\((0x[0-9a-f]+)\)/);
  const verIdx = verDecl ? parseInt(verDecl[2], 16) : inlineHost ? parseInt(inlineHost[5], 16) : null;
  const verName = verDecl ? verDecl[1] : inlineHost ? inlineHost[4] : '';

  const pool = locateStringPool(src);
  if (!pool) throw new Error('未能定位官方字符串池');

  const hostPos = pool.indexOf(anchor);
  if (hostPos < 0) throw new Error('字符串池中未找到校准地址');
  const offset = hostPos - hostIdx;
  const at = idx => (idx === null || idx === undefined ? undefined :
    pool[((idx + offset) % pool.length + pool.length) % pool.length]);
  ...
  // 平台前缀可能也是池内索引;从 prefix + version 的实际使用位置反查。
  const prefixDecl = src.match(/\w+='(win_|mac_)'/);
  const prefixUse = verName && src.match(new RegExp('\\b(\\w+)\\+' + verName + '\\['));
  const prefixIndex = prefixUse && src.match(new RegExp('\\b' + prefixUse[1] + '=\\w+\\((0x[0-9a-f]+)\\)'));
  const platformPrefix = prefixDecl ? prefixDecl[1] : prefixIndex ? at(parseInt(prefixIndex[1], 16)) : '';
  const appVersion = at(verIdx);
  if (!/^(win_|mac_)$/.test(platformPrefix)) throw new Error('未能还原客户端平台前缀');
  if (typeof appVersion !== 'string' || !/^\d+\.\d+\.\d+(?:[-+][\w.-]+)?$/.test(appVersion)) {
    throw new Error('未能还原客户端版本号');
  }
