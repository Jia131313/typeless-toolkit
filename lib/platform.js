...
function macLaunchSpec(exe, args) {
...
function appSpawnSpec(exe, args) {
  if (IS_MAC) return macLaunchSpec(exe, args || []);
...
function spawnDetachedChecked(exe, args) {
  return new Promise((resolve, reject) => {
    ...
    child.once('exit', code => {
      if (settled || (!IS_WIN && !spec.waitForExit)) return;
      settled = true;
      if (code === 0) resolve();
      else reject(new Error((IS_MAC ? 'macOS LaunchServices' : 'Windows Start-Process') + ' 启动失败，退出码 ' + code));
    });
...
// 杀进程 + 用调试端口重新启动(用于抓 token)。成功后返回实际使用的端口
// (配置端口被占用时会自动避让到空闲端口,调用方需用返回值继续 CDP 流程)。
async function taskRestartWithDebug(exe, cdpPort, userDataDir) {
  ...
  await sleep(400);
  // 调试端口避让:配置端口被占时 Chromium 会静默换随机端口,这里主动跳开避免死等。
  const preferredPort = Number(cdpPort) || 9222;
  const usePort = await findFreePort(preferredPort);
  const debugArgs = [`--remote-debugging-port=${usePort}`];
  ...
  for (let i = 0; i < 30; i++) {
    ...
    try {
      const response = await fetch(`http://127.0.0.1:${usePort}/json/version`, { signal: controller.signal });
      if (response.ok) return usePort;
    } catch (e) {
    } finally {
      clearTimeout(timer);
    }
  ...
  // 超时诊断:区分「端口被占」与「Typeless 没起来」,方便用户定位。
  const still = stillRunning();
  const occupied = await portOccupied(usePort);
  throw new Error(
    'Typeless 调试端口启动超时' +
    (usePort !== preferredPort ? `(配置端口 ${preferredPort} 被占用,已避让到 ${usePort})` : '') +
    (still ? ';Typeless 进程仍在运行,可能未监听调试端口' : ';Typeless 进程未在运行') +
    (occupied ? `;端口 ${usePort} 当前被其他程序占用` : '') +
    '。请确认 Typeless 能正常启动,或换一个 cdp_port 后重试'
  );
}
...
// TCP 端口占用探测(不含超时的裸 connect 会在 Windows 上挂很久,必须用短超时)
function portOccupied(port, timeoutMs = 500) {
...
// 在[preferred, preferred+step, ...]里挑一个未被占用的端口,用于 CDP 调试端口避让。
// 配置的 cdp_port 可能被浏览器/其他工具占用;Chromium 占位时静默换随机端口,
// 工具却仍死等配置端口,导致「调试端口启动超时」。这里主动探测并跳开。
async function findFreePort(preferred, step = 1, attempts = 20) {
  for (let i = 0; i < attempts; i++) {
    const candidate = preferred + i * step;
    if (candidate < 0 || candidate > 65535) break;
    if (!(await portOccupied(candidate))) return candidate;
  }
  return preferred; // 全部被占,返回原值,交给上层超时报错
}
...
function macCodeRequirement(codePath) {
  const target = String(codePath || '').includes(`${path.sep}Contents${path.sep}`)
    ? appBundleForExecutable(codePath)
    : codePath;
  const result = spawnSync('/usr/bin/codesign', ['-d', '-r-', target], { encoding: 'utf8' });
  const output = `${result.stdout || ''}\n${result.stderr || ''}`;
  const requirement = output.split(/\r?\n/).find(line => /designated\s*=>/.test(line));
  if (result.status !== 0 || !requirement) {
    throw new Error('无法读取 macOS 代码身份:' + (output.trim() || `codesign exit ${result.status}`));
  }
  return requirement.trim();
}

function permissionIdentityChanged(previous, current) {
...
const win = {
  ...
  killApp(exe, userDataDir) {
    ...
    // 清掉可能残留的单实例锁(与 mac 分支一致)。Chromium 进程死后 SingletonLock
    // 仍占着会让新实例立即退出,表现为「调试端口启动超时」。
    try {
  ...
  resetPrivacyPermissions() { return { skipped: true, reason: 'windows-no-tcc' }; },
...
const mac = {
  ...
  authAppName() { return 'typeless_webapp'; },
  appVersion(exe) {
    const plistPath = path.join(appBundleForExecutable(exe), 'Contents', 'Info.plist');
    return execFileSync('/usr/bin/plutil', [
      '-extract', 'CFBundleShortVersionString', 'raw', plistPath,
    ], { encoding: 'utf8' }).trim();
  },
  readAsarIntegrity(exe) {
    const plistPath = path.join(appBundleForExecutable(exe), 'Contents', 'Info.plist');
    let plist;
    try {
      plist = JSON.parse(execFileSync('/usr/bin/plutil', [
        '-convert', 'json', '-o', '-', plistPath,
      ], { encoding: 'utf8' }));
    } catch (error) {
      throw new Error('读取 Typeless ElectronAsarIntegrity 失败:' + commandFailureMessage(error));
    }
    const entry = plist && plist.ElectronAsarIntegrity && plist.ElectronAsarIntegrity['Resources/app.asar'];
    ...
    if (algorithm !== 'SHA256' || !/^[0-9a-f]{64}$/.test(hash)) {
      throw new Error('Typeless ElectronAsarIntegrity 的 app.asar 条目格式不受支持');
    }
    return { provider: 'macos-info-plist', algorithm, hash, plist_path: plistPath };
  },
  writeAsarIntegrity(exe, expectedHash, nextHash) {
    const current = this.readAsarIntegrity(exe);
    if (!current) throw new Error('Typeless Info.plist 中缺少 ElectronAsarIntegrity app.asar 条目');
    if (current.hash !== String(expectedHash || '').toLowerCase()) {
      throw new Error('Typeless Info.plist 的 app.asar hash 与当前 ASAR header 不一致');
    }
    execFileSync('/usr/libexec/PlistBuddy', [
      '-c', `Set :ElectronAsarIntegrity:Resources/app.asar:hash ${nextHash}`,
      current.plist_path,
    ], { stdio: 'ignore' });
    const updated = this.readAsarIntegrity(exe);
    if (updated.hash !== String(nextHash || '').toLowerCase()) {
      throw new Error('Typeless ElectronAsarIntegrity 更新后校验失败');
    }
    return updated;
  },
  exeCandidates() {
  ...
  launchApp(exe, cdpPort, userDataDir) {
    // 必须通过 LaunchServices 启动。直接 spawn Typeless 二进制会让 TCC 把工具集认作
    // responsible process，导致辅助功能/麦克风请求错误归到工具集名下。
    return spawnDetachedChecked(exe, macLaunchArgs(cdpPort, userDataDir));
  },
  resetPrivacyPermissions(bundleId, services) {
    const failures = [];
    const reset = [];
    for (const service of [...new Set((services || []).filter(Boolean))]) {
      try {
        execFileSync('/usr/bin/tccutil', ['reset', service, bundleId], { stdio: 'ignore' });
        reset.push(service);
      } catch (error) {
  ...
  resignApp(exe) {
    ...
    try {
      try { previousRequirement = macCodeRequirement(appBundle); } catch (error) {}
      ...
      const currentRequirement = macCodeRequirement(appBundle);
      const identityChanged = previousRequirement
        ? permissionIdentityChanged(previousRequirement, currentRequirement)
        : true;
      return {
        ...
        accessibility_regrant_required: identityChanged,
...
module.exports = {
  ...
  appBundleForExecutable, appBundleProcessPattern, macLaunchArgs, macLaunchSpec,
  macCodeRequirement, permissionIdentityChanged,
  portOccupied, findFreePort,
};
