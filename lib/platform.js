...
const win = {
  ...
  integrityPathFor(exe) { return exe; },
...
const mac = {
  ...
  asarPathFor(exe) { return path.join(path.dirname(exe), '..', 'Resources', 'app.asar'); },
  // Electron macOS 把 app.asar 整头 SHA256 存在 Info.plist 的 ElectronAsarIntegrity
  integrityPathFor(exe) { return path.join(path.dirname(exe), '..', 'Info.plist'); },
  ...
  // 打包后的 Electron 会把任意 .asar 路径当归档处理；用系统 cp 读取归档文件本体
