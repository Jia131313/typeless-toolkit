...
// 兼容早期版本把 refresh token 保存到 token 字段的账号记录。
function storedRefreshToken(account) {
  if (account && account.refresh_token) return account.refresh_token;
  return tokenType(account && account.token) === 'refresh' ? account.token : null;
}

// 登录可持续时间由 refresh token 决定；老版本只有 access token 时保持兼容。
function effectiveCredentialExpiryMs(account) {
  return tokenExpiryMs(storedRefreshToken(account))
...
function createAccountCredentialManager({
  ...
  async function ensureAccessToken(account) {
    ...
    const nowMs = nowFn();
    // 旧记录的 token 可能是 refresh token，不能把它直接当作 access token 调接口。
    const accessToken = tokenType(account.token) === 'refresh' ? null : account.token;
    const accessExpiry = tokenExpiryMs(accessToken);
    if (accessToken && accessExpiry && accessExpiry > nowMs + refreshAheadMs) return accessToken;
    const refreshToken = storedRefreshToken(account);
    if (!refreshToken) {
      if (accessToken && accessExpiry && accessExpiry > nowMs) return accessToken;
...
function createLiveStatus({ ensureAccessTokenFn, curlApiFn, nowFn = Date.now }) {
  return async function accountLiveStatus(acc) {
    const out = {
      ...
      token_exp: tokenType(acc.token) === 'refresh' ? null : tokenExpiryMs(acc.token), token_days_left: null,
      access_exp: tokenType(acc.token) === 'refresh' ? null : tokenExpiryMs(acc.token),
      refresh_exp: tokenExpiryMs(storedRefreshToken(acc)),
      credential_exp: effectiveCredentialExpiryMs(acc), credential_days_left: null,
      credential_state: storedRefreshToken(acc) ? 'ready' : 'legacy',
    };
    ...
    out.refresh_exp = tokenExpiryMs(storedRefreshToken(acc));
