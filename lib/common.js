...
// ---------- 弹窗补丁(两层 asar 完整性) ----------
// 候选脚本:渲染层 .mjs/.js(2.0.1 起 hash 名可能变成 .js)
...
// 定位真正承载 paywall 弹窗处理的文件(同时含 onImportantNotification + onSessionInterrupt + type==='paywall')
function locatePaywallTarget(header, buf, dataStart) {
  const configuredPath = (config.paywall && config.paywall.file_path) || [];
  const configuredKey = configuredPath.join('/');
  const paths = [];
  if (configuredKey) paths.push(configuredKey);
  if (config.paywall && config.paywall.auto_detect_file !== false) {
    for (const rel of detectPaywallFile(header)) paths.push(rel);
  }
  const seen = new Set();
  let best = null;
  ...
  for (const rel of paths) {
    ...
    const content = buf.subarray(dataStart + (+node.offset), dataStart + (+node.offset) + node.size).toString('utf8');
    const configured = (config.paywall && config.paywall.replacements) || [];
    const hasConfiguredMarker = configured.some(([from, to]) => content.includes(from) || content.includes(to));
    // 真正的弹窗处理文件特征:两处 handler + type==='paywall' 分支调用
    const hasHandlerShape = content.includes('onImportantNotification') &&
      content.includes('onSessionInterrupt') &&
      /\[(?:'type'|"type")\]===(?:'paywall'|"paywall")/.test(content);
    const score = (hasHandlerShape ? 100 : 0) + (hasConfiguredMarker ? 50 : 0) +
      (content.includes('paywall') ? 5 : 0) + (rel === configuredKey ? 1 : 0);
    if (score < 100 && !hasConfiguredMarker) continue; // 仅含埋点/文案里的 paywall 不算
...
// 从当前混淆代码推导等长替换;config 标记仍优先,版本升级后自动适配 mn/gn/_n 等函数名
function resolvePaywallReplacements(content) {
  ...
  // 未补丁形态:if(x['type']==='paywall')fn(x)  → 等长替换 fn(x) 为 (0,x)
  ...
  if (unique.length < 2) {
    throw new Error(
      '已定位弹窗处理文件,但无法推导两个等长替换标记;当前 Typeless 版本结构可能已变化。' +
      '请阅读 README「弹窗补丁适配」章节,用 DevTools 定位后填入 config.json'
    );
  }
  ...
  try {
    ...
    if (!target) {
      return {
        ...
        error: 'asar 内未找到弹窗处理文件(config.paywall.file_path 不匹配,且自动探测未找到 onImportantNotification/onSessionInterrupt 处理器)。' +
               '请阅读 README「弹窗补丁适配」章节,定位你版本的文件后填入 config.json',
      };
    }
    const { filePath, node, detected, content: contentText } = target;
    const content = buf.subarray(dataStart + (+node.offset), dataStart + (+node.offset) + node.size);
    let resolved;
    try { resolved = resolvePaywallReplacements(contentText); }
    catch (e) {
      return {
        exists: true, patched: false, detected_file: detected ? filePath.join('/') : null,
        file_path: filePath.join('/'), error: e.message,
        has_backup: fs.existsSync(ASAR_PATH + '.bak') && fs.existsSync(TYPELESS_EXE + '.bak'),
      };
    }
    const repls = resolved.replacements;
    const hasOld = repls.length > 0 && repls.every(([from]) => content.includes(Buffer.from(from, 'utf8')));
    const hasNew = repls.length > 0 && repls.every(([, to]) => content.includes(Buffer.from(to, 'utf8')));
    return {
      exists: true,
...
function patchPaywall() {
  ...
  // 大版本更新后旧 .bak 可能过期,仅在不存在时创建,避免覆盖用户可用回滚点
  ...
  try {
    ...
    if (resolved.alreadyPatched) {
      ...
      return {
        ...
        msg: '已是无弹窗补丁版,无需重复操作',
        file_path: filePath.join('/'),
        detected_replacements: resolved.detected ? resolved.replacements : null,
      };
    }
    ...
    for (const [from, to] of repls) {
      if (from.length !== to.length) throw new Error('替换必须等长: ' + from.toString() + ' → ' + to.toString());
    ...
    return {
      ...
      detected_replacements: resolved.detected,
...
module.exports = {
  ...
  resolvePaywallReplacements, // 供单测 / 版本探测
};
