...
// 提取失败时降级为不带签名(保持改造前的行为,由响应体暴露真实原因)。
let apiSignatureKeys;
let apiSignatureError = '';
function getApiSignatureKeys() {
  if (apiSignatureKeys !== undefined) return apiSignatureKeys;
  apiSignatureKeys = null;
  try {
    ...
    try { apiSignatureKeys = apiSecurity.extractSignatureKeys(fs.readFileSync(tmpAsar)); }
    finally { try { fs.unlinkSync(tmpAsar); } catch (e) {} }
  } catch (e) {
    apiSignatureError = e.message;
  }
  return apiSignatureKeys;
}
/** 供 UI/诊断使用:当前是否具备客户端签名能力 */
function apiSignatureStatus() {
  const keys = getApiSignatureKeys();
  return keys
    ? { ok: true, app_version: keys.platformPrefix + keys.appVersion }
    : { ok: false, error: apiSignatureError || '未提取' };
}
...
async function curlApi(method, p, token, body) {
  ...
  if (token && String(API_BASE).includes('api.typeless.com') && apiSecurity.shouldSignApiPath(p)) {
    try {
      const keys = getApiSignatureKeys();
      const userId = keys && tokenUserId(token);
      if (userId) {
