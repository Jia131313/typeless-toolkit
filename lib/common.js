...
/** 综合 app-onboarding.json 与 app-storage 判断是否已完成 */
function checkOnboardingStatus() {
  ...
  const localDone = !!(onboarding && onboarding.isCompleted);
  const storageDone = !!(
    storage &&
    storage.userData &&
    storage.userData.onboarding &&
  ...
  return {
    completed: localDone || storageDone,
    local_completed: localDone,
    storage_completed: storageDone,
    step: (onboarding && onboarding.step) ?? 0,
    user_id: storage && storage.userData && storage.userData.user_id || null,
    platform: platformKey,
  };
}
...
async function syncAccount(acc) {
  ...
  let accountWords = (dl0.words || []).map(w => w.term).filter(Boolean);
  ...
  let totalImported = 0;
  let remaining = termsMissingFrom(masterMerged, accountWords);
  ...
  for (let round = 1; round <= SYNC_MAX_ROUNDS && remaining.length; round++) {
    ...
    accountWords = (dl.words || []).map(w => w.term).filter(Boolean);
    const still = termsMissingFrom(masterMerged, accountWords);
    rounds.push({
      round,
  ...
  const after_count = accountWords.length;
  ...
  return {
    ...
    before_count,
    after_count,
    master_count: masterMerged.length,
    missing_after: remaining.length,
    missing_terms: remaining.slice(0, 30),
    aligned,
    rounds,
    msg: aligned
      ? `已对齐主库 ${masterMerged.length} 词(本号 ${after_count})`
      : `未完全对齐:仍缺 ${remaining.length} 词(主库 ${masterMerged.length},本号 ${after_count})`,
  };
}
...
async function syncAllAccounts() {
  ...
  for (const a of accs) {
    try {
      results.push(await syncAccount(a));
    } catch (e) {
      results.push({
        user_id: a.user_id,
        nickname: a.nickname,
  ...
  // 第二遍快速收敛:master 可能在第一遍中被后面账号扩充
  const master = readMaster();
  for (let i = 0; i < results.length; i++) {
    const r = results[i];
    if (r.error || r.aligned) continue;
    const acc = accs.find(x => x.user_id === r.user_id);
    if (!acc) continue;
    try {
      const again = await syncAccount(acc);
      results[i] = again;
    } catch (e) {
      results[i] = { ...r, error: e.message, aligned: false };
    }
  }
  ...
  return {
    ...
    msg: failed === 0
      ? `全部 ${accs.length} 个账号已与主库(${master.length} 词)对齐`
...
module.exports = {
  ...
  // 状态 + 同步 + 备份
  liveStatus, syncAccount, syncAllAccounts, bulkImportTerms, backupData,
  // 弹窗补丁
