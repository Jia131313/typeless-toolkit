...
function clearDictionaryDeletions(terms) {
  ...
  for (const term of terms || []) {
    ...
    if (key && !meta.active[key]) meta.active[key] = { term: String(term).trim(), updated_at: new Date().toISOString() };
...
function createAccountCredentialManager({
  ...
  async function refreshAccount(account) {
    ...
    const updated = {
      ...
      client_user_id: checked.client_user_id || account.client_user_id || null,
    ...
    const next = index >= 0
      ? accounts.map((item, i) => i === index ? { ...item, ...updated } : item)
      : accounts.concat(updated);
