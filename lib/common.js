...
const accountCredentialManager = createAccountCredentialManager({
  ...
  refreshRequestFn: async (refreshToken, appName) => {
    const response = await curlApi(
      'POST',
      `/oauth/refresh_access_token?app=${encodeURIComponent(appName)}`,
      refreshToken,
    );
...
module.exports = {
  ...
  createAccountCredentialManager, ensureAccountAccessToken, ensureAccountCredentials,
  // 状态 + 同步 + 备份
