...
async function captureTokenCDP(port, autoRestart = true) {
  ...
  // 探测当前状态
  let portReady = false;
  try { portReady = (await fetch(`http://127.0.0.1:${usePort}/json/version`)).ok; } catch (e) {}
  let typelessWasRunning = await PLAT.isAppRunning(TYPELESS_EXE);

  if (!portReady) {
    // autoRestart=false(仅做状态探测)时不杀 Typeless,避免打断用户正在用的 Typeless
    if (!autoRestart) throw new Error('Typeless 未以调试端口运行');
    // 启动 Typeless 带调试端口(ensureApp 内部先 kill 旧进程再启动)
  ...
  // 抓完后恢复干净模式(杀掉带调试端口的 Typeless,重新干净启动)
  // 这样 Typeless 不会常驻调试端口,用户无感知
  try {
