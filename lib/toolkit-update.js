...
function normalizedMacArch(value) {
  if (value === 'arm64' || value === 'aarch64') return 'arm64';
  if (value === 'x64' || value === 'x86_64') return 'x64';
  return null;
}

function assetNames(version, platform, flavor, arch) {
  if (platform === 'win32') {
  ...
  if (platform === 'darwin') {
    const macArch = normalizedMacArch(arch);
    if (!macArch || !['portable', 'lite'].includes(flavor)) return null;
    const archive = `Typeless-Toolkit-${version}-mac-${macArch}-${flavor}.dmg`;
    return { archive, checksum: `${archive}.sha256.txt` };
  }
  return null;
}
...
function releaseSummary(release, version, currentVersion, platform, flavor, arch) {
  const names = assetNames(version, platform, flavor, arch);
  ...
  return {
    ...
    flavor,
    arch: platform === 'darwin' ? normalizedMacArch(arch) : 'x64',
    asset: archive ? {
      name: archive.name,
    ...
    error: !names
      ? `当前${platform === 'darwin' ? ' macOS 架构或安装类型' : ' Windows 安装类型'}无法确认，请从 Release 手动升级`
...
function powershell(command, args, options = {}) {
  ...
  const literals = args.map(value => "'" + String(value).replaceAll("'", "''") + "'").join(' ');
  const invocation = `& { ${command} } ${literals}`;
...
try {
  ...
  $newProcess = Start-Process -FilePath $RestartExe -WorkingDirectory $install -ArgumentList ('--toolkit-update-ready "{0}" --toolkit-update-version {1}' -f $ready, $TargetVersion) -PassThru
...
function createToolkitUpdateController(options = {}) {
  ...
  const arch = platform === 'darwin' ? normalizedMacArch(options.arch || process.arch) : 'x64';
  ...
  const state = {
    ...
    flavor, arch, progress: null, error: null, update: null, download_path: null,
  ...
  async function check() {
    ...
    try {
      ...
      const update = releaseSummary(release, version, currentVersion, platform, flavor, arch);
...
function readUpdateResult(dataRoot) {
  try {
    const value = JSON.parse(fs.readFileSync(path.join(dataRoot, 'toolkit-update-result.json'), 'utf8').replace(/^\uFEFF/, ''));
